...
def test_actions_for_online_model_free_training() -> None:
    ...
    np.testing.assert_array_equal(
        ...
        [1] * random_steps + [0] * (total_steps - random_steps),
    )
    np.testing.assert_array_equal(
...
def test_actions_for_online_model_based_training() -> None:
    ...
    np.testing.assert_array_equal(
        ...
        + [0] * (total_steps - init_steps - 3 * train_model_every - 1),
    )
    np.testing.assert_array_equal(
        action_log[:, Actions.UPDATE_AGENT],
        [0] * init_steps + [1] * (total_steps - init_steps),
...
def test_actions_for_offline_model_free_training_with_fine_tuning():
    ...
    np.testing.assert_array_equal(
        test_performances.shape, (epochs + pretrain_epochs, 2)
    )
    ...
    np.testing.assert_array_equal(
        ...
        [0] * steps_per_epoch * pretrain_epochs + [1] * steps_per_epoch * epochs,
    )
...
def test_actions_for_offline_model_based_training_with_fine_tuning() -> None:
    ...
    trainer = Trainer(
        ...
        pretrain_epochs=pretrain_epochs,
        agent_kwargs=dict(type="sac", hidden=[32, 32, 32], batch_size=32),
        model_kwargs=dict(hidden=[32, 32], batch_size=32, patience=0),
        use_model=True,
        train_model_every=train_model_every,
        steps_per_epoch=steps_per_epoch,
        max_ep_len=30,
