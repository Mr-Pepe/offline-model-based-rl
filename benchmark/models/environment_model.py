...
class EnvironmentModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.max_logvar = torch.ones(n_networks, (self.out_dim), device=device) * 0.5
        self.min_logvar = torch.ones(n_networks, (self.out_dim), device=device) * -10
    ...
    def train_to_convergence(self, data, lr=1e-3, batch_size=1024,
        ...
        while n_bad_val_losses < patience and (max_n_train_epochs == -1 or epoch < max_n_train_epochs):
            ...
            for i in range(n_train_batches):
                ...
                if mean is not None:
                    self.max_logvar += (torch.log(mean.detach().var(dim=1)*10) - self.max_logvar)*0.001
