...
class CopyCat(nn.Module):
    ...
    def multi_update(self, n_updates, buffer, logger=None, debug=False):
        ...
        if self.knn is None:
            self.knn = buffer.get_knn(k=k, pre_fn=self.pre_fn)

        losses = torch.zeros(n_updates)
        for i_update in range(n_updates):
            act_idx = torch.randint(0, k, (self.batch_size,))
            batch, idxs = buffer.sample_batch(self.batch_size, return_idxs=True)

            for i_idx, idx in enumerate(idxs):
                batch['act'][i_idx] = buffer.act_buf[int(self.knn[idx][act_idx[i_idx]])]
