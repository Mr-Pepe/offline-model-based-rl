...
from offline_mbrl.utils.mazes import (
    ...
    ANTMAZE_MEDIUM_WALLS_WITHOUT_OUTSIDE_CUDA,
    ANTMAZE_UMAZE_MAX,
    ANTMAZE_UMAZE_MIN,
    ANTMAZE_UMAZE_WALLS_WITHOUT_OUTSIDE,
    ANTMAZE_UMAZE_WALLS_WITHOUT_OUTSIDE_CUDA,
    MAZE2D_POINT_RADIUS,
    MAZE2D_UMAZE_WALLS,
)
...
def postprocess_antmaze_umaze(next_obs=None, means=None, logvars=None, **_):
    ...
    collision = (
        (
            ...
            * (walls[:, 3] > y.unsqueeze(-1) - ANTMAZE_ANT_RADIUS)
        ).sum(dim=2)
        + (maze_max <= x)
        + (x <= maze_min)
        + (maze_max <= y)
    ...
    notdone = (
        torch.isfinite(next_obs).all(dim=2)
        * (next_obs[:, :, 2] >= 0.2)
        * (next_obs[:, :, 2] <= 1.0)
    )
    done = ~notdone
...
def postprocess_antmaze_medium(
    next_obs=None, means=None, logvars=None, ant_radius=None, **_
):
    x = next_obs[:, :, 0]
    y = next_obs[:, :, 1]
    ...
    collision = (
        (
            ...
            * (walls[:, 3] > y.unsqueeze(-1) - ant_radius)
        ).sum(dim=2)
        + (maze_max <= x)
        + (x <= maze_min)
        + (maze_max <= y)
...
def postprocess_maze2d_umaze(next_obs=None, means=None, logvars=None, **_):
    ...
    collision[:, :, -1] = (x_max <= x) + (x <= x_min) + (y_max <= y) + (y <= y_min)
    ...
    return {
        "dones": collision.unsqueeze(-1).to(x.device),
        "means": means,
        "logvars": logvars,
    }
