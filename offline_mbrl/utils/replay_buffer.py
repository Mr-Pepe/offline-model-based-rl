...
class ReplayBuffer:
    ...
    def store_batch(self, obs, act, rew, next_obs, done):
        ...
        self.obs_buf[self.ptr : self.ptr + remaining_batch_size] = obs[
            -remaining_batch_size:
        ]
        self.obs2_buf[self.ptr : self.ptr + remaining_batch_size] = next_obs[
            -remaining_batch_size:
        ]
        self.act_buf[self.ptr : self.ptr + remaining_batch_size] = act[
            -remaining_batch_size:
        ]
        self.rew_buf[self.ptr : self.ptr + remaining_batch_size] = rew[
            -remaining_batch_size:
        ]
        self.done_buf[self.ptr : self.ptr + remaining_batch_size] = done[
            -remaining_batch_size:
        ]
    ...
    def sample_batch(self, batch_size=32, non_terminals_only=False, return_idxs=False):
        ...
        batch = dict(obs=obs, obs2=obs2, act=act, rew=rew, done=done)
    ...
    def sample_balanced_terminal_batch(self, batch_size=32, val_split=0.2):
        ...
        return dict(
            obs=torch.cat((self.obs_buf[done_idxs], self.obs_buf[not_done_idxs])),
            obs2=torch.cat((self.obs2_buf[done_idxs], self.obs2_buf[not_done_idxs])),
            act=torch.cat((self.act_buf[done_idxs], self.act_buf[not_done_idxs])),
            rew=torch.cat((self.rew_buf[done_idxs], self.rew_buf[not_done_idxs])),
            done=torch.cat((self.done_buf[done_idxs], self.done_buf[not_done_idxs])),
        )
    ...
    def get_knn_naive(self, k=3, pre_fn=None):
        ...
        for i in range(len(knn)):
            this_knn = torch.topk(
                torch.linalg.norm((obs - obs[i, :]), dim=(1,)), k, largest=False
            )
    ...
    def get_knn(self, k=3, pre_fn=None, query=None, verbose=False, batch_size=20):
        ...
        knn = torch.empty((len(obs), k), device=self.obs_buf.device)
        ...
        while i < len(obs):
            ...
            this_knn = torch.topk(
                torch.linalg.norm(obs[None, :, :] - obs[i:i_to, None, :], dim=2),
                k,
                largest=False,
            )
